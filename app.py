...
# # # --- 2. ASYNC BRIDGE (Streamlit to LangGraph) ---
# # async def run_research_graph(inputs, thread_id, status_placeholder, brief_placeholder):
# #     config = {"configurable": {"thread_id": thread_id}}
# #     final_state = dict(inputs) 
    
# #     # Track mode locally so it doesn't "flicker"
# #     current_active_mode = inputs.get("mode", "fast") 
...
# #             # 1. Update the local mode if the node provides it
# #             if "mode" in output:
# #                 current_active_mode = output["mode"]
# #             elif "research_mode" in output:
# #                 current_active_mode = output["research_mode"]

# #             # 2. Update the UI with the "Locked" mode
# #             mode_label = "🚀 FAST MODE" if current_active_mode == "fast" else "🌊 DEEP MODE"
# #             mode_color = "green" if current_active_mode == "fast" else "blue"

# #             with status_placeholder:
# #                 if node_name == "scoping":
# #                     brief_placeholder.write(f":{mode_color}[{mode_label}]: Analyzing intent...")
# #                 elif node_name == "research_brief":
# #                     brief_placeholder.write(f":{mode_color}[{mode_label}]: Building deep-dive questions...")
# #                 elif node_name == "deep_research" or node_name == "research":
# #                     brief_placeholder.write(f":{mode_color}[{mode_label}]: Researching live data...")
# #                 elif node_name == "synthesis":
# #                     brief_placeholder.write(f":{mode_color}[{mode_label}]: Generating final report...")

# #             # 3. Handle the Brief/Question streaming
# #             if "last_completed_question" in output:
# #                 with brief_placeholder:
# #                     st.success(f"📍 **Research Point:** {output['last_completed_question']}")
# #             if "active_task_description" in output:
# #                 current_task = output["active_task_description"]
                
# #                 # Now render it in your placeholder
# #                 with brief_placeholder:
# #                     # Using st.chat_message or st.info makes it stand out
# #                     st.info(f"🔎 **Agent is researching:** {current_task}")
                    
# #                     # Optional: Add to a log so they don't disappear
# #                     if "task_log" not in st.session_state:
# #                         st.session_state.task_log = []
# #                     st.session_state.task_log.append(current_task)
# #     return final_state
# # # async def run_research_graph(inputs, thread_id, status_placeholder, brief_placeholder):
# # #     # --- 1. INITIAL HARD RESET ---
# # #     st.session_state.task_log = []
...
# # #     displayed_content_hashes = set()
# # #     config = {"configurable": {"thread_id": thread_id}}
# # #     final_state = dict(inputs) 
    
# # #     current_active_mode = inputs.get("mode", "fast") 

# # #     # --- 2. ASYNC STREAMING LOOP ---
# # #     async for chunk in graph_app.astream(inputs, config=config, stream_mode="updates"):
# # #         for node_name, output in chunk.items():
# # #             final_state.update(output)
            
# # #             # 3. MODE & LABEL SYNC
# # #             if "research_mode" in output:
# # #                 current_active_mode = output["research_mode"]
# # #             elif "mode" in output:
# # #                 current_active_mode = output["mode"]
...
# # #             # 4. UPDATE STATUS BAR (Top Level Progress)
# # #             with status_placeholder:
# # #                 if node_name == "scoping":
# # #                     st.markdown(f":{mode_color}[**{mode_label}**]: Analyzing your travel intent...")
# # #                 elif node_name == "research_brief":
# # #                     st.markdown(f":{mode_color}[**{mode_label}**]: Building deep-dive research plan...")
# # #                 elif node_name in ["deep_research", "research"]:
# # #                     st.markdown(f":{mode_color}[**{mode_label}**]: Scanning live sources and travel databases...")
# # #                 elif node_name == "synthesis":
# # #                     st.markdown(f":{mode_color}[**{mode_label}**]: Finalizing your custom report...")

# # #             # 5. DYNAMIC LOG RENDERING
# # #             with brief_placeholder.container():
# # #                 if current_active_mode == "fast":
# # #                     if "raw_research_output" in output:
# # #                         for res in output["raw_research_output"]:
# # #                             content = res.get("content", "")
# # #                             if content:
# # #                                 displayed_content_hashes.add(content)

# # #                     if displayed_content_hashes:
# # #                         st.markdown("### ⚡ Quick Research Findings")
# # #                         for item in displayed_content_hashes:
# # #                             st.success(f"💡 {item}")
...
# # #                         # Reset log when the planning phase is over
# # #                         if node_name == "research_brief":
# # #                              st.session_state.task_log = [] 

# # #                         if current_task not in st.session_state.task_log:
# # #                             st.session_state.task_log.append(current_task)
                        
# # #                         st.markdown("### 🌊 Deep Research Pipeline")
# # #                         st.info(f"🔎 **Currently Processing:** {current_task}")
                        
# # #                         if len(st.session_state.task_log) > 1:
# # #                             with st.expander("View Completed Mission Steps", expanded=False):
# # #                                 for task in st.session_state.task_log[:-1]:
# # #                                     st.write(f"✅ {task}")
                        
# # #                         st.spinner("Processing deep-dive queries...")
...
# #         # UI Toggle for Speed/Depth
# #         st.subheader("⚙️ Agent Settings")
# #         selected_mode = st.toggle("Enable Deep Research Mode", value=False, help="Fast mode uses quick API hits. Deep mode performs multi-step verification.")
# #         research_mode = "deep" if selected_mode else "fast"
...
# #         if history:
# #             for m_id, query, _ in history:
# #                 if st.button(f"📄 {query[:20]}...", key=str(m_id)):
# #                     report = load_past_research(m_id)
# #                     st.session_state.messages = [{"role": "assistant", "content": report}]
# #                     st.session_state.current_report = report
# #                     st.session_state.current_mission_id = str(m_id)
...
# #     # Chat Input
# #     if prompt := st.chat_input("Ex: Plan a 5-day luxury trip to Kyoto"):
# #         st.session_state.messages.append({"role": "user", "content": prompt})
# #         with st.chat_message("user"):
# #             st.markdown(prompt)
...
# #             "user_id": st.session_state.user_id,
# #             "mission_id": st.session_state.current_mission_id,
# #             "mode": research_mode  # Pass user's choice to the graph
# #         }

# #         with st.chat_message("assistant"):
# #             # UI Containers for streaming updates
# #             status_area = st.empty()
# #             brief_area = st.container() 
...
# #             # Final Logic for Report or Clarification
# #             needs_clarify = final_state.get("clarification_needed", False)
# #             question = final_state.get("clarification_question")
# #             report = final_state.get("final_report")

# #             if needs_clarify and question:
# #                 st.warning(f"**Action Required:** {question}")
# #                 st.session_state.messages.append({"role": "assistant", "content": question})
...
# #     # Download Button
# #     if st.session_state.current_report:
# #         st.download_button("📥 Download Report", st.session_state.current_report, file_name="itinerary.txt")
...
#     # Track mode and tasks in session state to persist through the stream
#     if "task_log" not in st.session_state:
#         st.session_state.task_log = []
...
#             # Determine mode for UI styling
#             mode = output.get("research_mode") or final_state.get("research_mode", "fast")
#             mode_label = "🚀 FAST MODE" if mode == "fast" else "🌊 DEEP MODE"
#             mode_color = "green" if mode == "fast" else "blue"

#             # 1. Update the Status Area (Top Level Phase)
#             status_placeholder.markdown(f"### :{mode_color}[{mode_label}]")
            
#             # 2. Handle the Brief/Question streaming using a container to prevent flickering
#             with brief_placeholder.container():
#                 if node_name == "scoping":
#                     st.write("🔍 **Analyzing intent and checking dimensions...**")
                
#                 if output.get("active_task_description"):
#                     current_task = output["active_task_description"]
#                     if current_task not in st.session_state.task_log:
#                         st.session_state.task_log.append(current_task)
...
#                 if len(st.session_state.task_log) > 1:
#                     with st.expander("View Research Progress", expanded=False):
#                         for task in st.session_state.task_log[:-1]:
#                             st.write(f"✅ {task}")
...
#         st.subheader("⚙️ Agent Settings")
#         selected_mode = st.toggle("Enable Deep Research Mode", value=False)
#         research_mode = "deep" if selected_mode else "fast"
...
#     # Chat Input Logic
#     if prompt := st.chat_input("Ex: Plan a 5-day luxury trip to Kyoto"):
#         # 1. Stitch Context if answering a clarification
#         query_to_send = prompt
#         if st.session_state.awaiting_clarification and len(st.session_state.messages) > 0:
#             # Find the original query (the last user message before the current answer)
#             original_query = next((m["content"] for m in reversed(st.session_state.messages) if m["role"] == "user"), "")
#             query_to_send = f"Original Request: {original_query} | Clarification provided: {prompt}"
...
#         with st.chat_message("assistant"):
#             status_area = st.empty()
#             brief_area = st.empty() # Placeholder for streaming tasks
...
#                 st.session_state.messages.append({"role": "assistant", "content": full_response})
#                 st.session_state.current_report = full_response
#                 save_final_report_to_db(st.session_state.user_id, st.session_state.current_mission_id, query_to_send, full_response)

#     if st.session_state.current_report:
#         st.download_button("📥 Download Itinerary", st.session_state.current_report, file_name="travel_itinerary.txt")

# if __name__ == "__main__":
#     if "logged_in" not in st.session_state: st.session_state.logged_in = False
#     if not st.session_state.logged_in: auth_gate()
#     else: research_interface()    
import streamlit as st
import time
import uuid
import asyncio
...
# --- 2. ASYNC BRIDGE (Streamlit to LangGraph) ---
async def run_research_graph(inputs, thread_id, status_placeholder, brief_placeholder):
    # Clear the UI area immediately upon starting
    brief_placeholder.empty()
    st.session_state.task_log = []
    ...
    async for chunk in graph_app.astream(inputs, config=config, stream_mode="updates"):
        for node_name, output in chunk.items():
            ...
            # 1. Update the Status Area
            status_placeholder.markdown(f"### :{mode_color}[{mode_label}]")
            
            # 2. Handle the Brief/Question streaming
            with brief_placeholder.container():
                if node_name == "scoping":
                    st.write("🔍 **Analyzing intent and checking dimensions...**")
                
                # UI GUARD: Only display if there is a real task and it isn't "None"
                ...
                if current_task and str(current_task).lower() != "none":
                    ...
                    st.info(f"🔎 **Agent is researching:** {current_task}")
                elif current_task is None and mode == "fast":
                    # In Fast Mode, if task is None, clear the gap display entirely
                    brief_placeholder.empty()
...
def research_interface():
    ...
    if prompt := st.chat_input("Ex: Plan a 5-day luxury trip to Kyoto"):
        ...
        # --- PRE-FLIGHT WIPE ---
        # Clear research gaps from previous queries before the new one starts
        if not st.session_state.awaiting_clarification:
            st.session_state.task_log = []
            # Resetting the placeholder content implicitly by re-running
        ...
        with st.chat_message("assistant"):
            ...
            if needs_clarify and question:
                ...
                st.session_state.awaiting_clarification = False
                # Wipe the research gap area before showing the final report
                brief_area.empty()
                full_response = st.write_stream(report_generator(report))
                st.session_state.messages.append({"role": "assistant", "content": full_response})
                st.session_state.current_report = full_response
